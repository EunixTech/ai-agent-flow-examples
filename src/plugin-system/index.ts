...
export class WeatherNode extends Node {
  ...
  async execute(context: Context): Promise<NodeResult> {
    console.log(`WeatherNode: Getting weather for ${context.data.location || 'default location'}`);
...
export class AnthropicProvider {
  ...
  constructor(apiKey: string) {
    ...
    this.client = {
      messages: {
        create: async () => ({ content: [{ text: 'This is a simulated response' }] })
      }
    };
  }
...
async function runExample() {
  ...
  flow.addTransition('get-weather', { action: 'default', to: 'process-weather' });
